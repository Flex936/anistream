...
- **Collapsing scroll headers** use `SliverPersistentHeaderDelegate`, not `NestedScrollView` — reserve `NestedScrollView` for a screen that genuinely has a second, independently-scrolling inner region; a single collapsing header inside an existing `CustomScrollView` doesn't need it. `AnimeDetailsScreen`'s `HeroHeaderDelegate` (`features/anime_details/widgets/hero_header_delegate.dart`) is the canonical example: `maxExtent`/`minExtent` of 280/72, cross-fading between a "full" and "compact" widget variant via `shrinkOffset`. Under `uiPerformanceMode`, the delegate hard-swaps at the midpoint instead of cross-fading every scrolled pixel, per § 2's animation-duration rule — continuous per-frame `Opacity` interpolation on a pinned header is exactly the cost that rule exists to strip.
