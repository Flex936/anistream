...
- Source poster card sizing from `lib/core/theme/app_card_sizes.dart` (`context.appCardSizes`) rather than a per-screen literal — `posterAspectRatio` (2:3, matching AniList's own `coverImage` art exactly) is the single canonical ratio every poster card renders at (`AnimeCard`, `WatchlistCard`, `ListCard`, `CalendarCard`); `shelfWidth` (170px) is the fixed width every horizontal shelf/carousel (`AnimeCarousel`, `ScheduledScreen`'s day shelves) uses; `gridMaxWidth`/`heroGridMaxWidth` cap how wide a single card in a fluid grid (`SearchResultsScreen`, `WatchlistScreen`) is allowed to grow on a wide viewport — see § 3 for how this feeds grid column counts. `HeroCard`'s 16:9 landscape treatment is a deliberate exception (it renders banner art, not cover art) and doesn't use `posterAspectRatio`.
...
- Blur sigma is drawn from three named tiers defined in `lib/core/theme/app_materials.dart` (`AppMaterials`), accessed via `context.appMaterials`, matching the same `ThemeExtension` pattern as `AppRadii`/`AppTypography`/`AppCardSizes`: **`.subtle` (10px)** for small controls (badges, icon buttons, floating pill buttons), **`.standard` (16px)** for content surfaces (dropdowns, popups, menus, full-screen loading overlays), and **`.prominent` (40px)** for large panels (side drawers, control bars, toasts).
- **A full-screen glassmorphic backdrop behind a centered-card modal** — `.prominent` for the backdrop itself, `.standard` for the card — is this layer's canonical treatment for centered modals, alongside the side-panel pattern § 1.1 already covers. Both concrete examples share one implementation, `SelectionModal` (`shared/widgets/selection_modal.dart`), which takes the glass treatment as an explicit `useGlassEffect` flag rather than assuming every centered modal wants it: `TorrentSearchModal` (`features/anime_details/widgets/torrent_search_modal.dart`) sets it `true` and gets a tap-to-dismiss backdrop, since it's mounted as a real pushed route (`showGeneralDialog<Torrent>` — selecting a release pops the route with that `Torrent` as its result, the same `showDialog<T>`-style contract Flutter's own dialogs use, rather than a callback invoked while the route is still open); `BatchEpisodePickerOverlay` (`features/theater/widgets/batch_picker.dart`) sets it `false` — it predates the `AppMaterials` tiers and is tracked as an outlier in § 5.3 rather than silently migrated, and it's mounted inline as one branch of `TheaterScreen`'s own loading/selection/ready state machine rather than as a pushed route, so it has no backdrop-dismiss or pop-with-value behavior to speak of. Because `TorrentSearchModal`'s backdrop is a real, opaque widget painted in front of the route's own (transparent) `ModalBarrier` rather than relying on `barrierColor`, it carries its own tap-to-dismiss handler — `showGeneralDialog`'s `barrierDismissible` alone won't reach it.
...
- Shared breakpoints live in `build_context_extensions.dart` (`Breakpoints.{mobile,tablet,desktop,wide}` = 600/900/1200/1500), for layout branches that need a true mobile/tablet/desktop split — `context.isMobile`, mobile-only widget swaps, mobile-only nav treatments. New screens should use `context.isMobile` rather than a new hardcoded `< 600` check — § 5 tracks the current split between migrated and not-yet-migrated call sites.
- Every `autofocus:` condition, and every `DpadFocusable` `builder` that styles itself off `state.focused`, must be routed through `context.dpadAutofocus(...)`/`context.dpadModeActive` (`core/input/input_mode_scope.dart`) rather than a raw boolean or raw `state.focused` — this is what actually enforces "Display visual focus rings ONLY when `dpadModeActive` is true" above. `HoverFocusBuilder` is the reference implementation: it separates genuine mouse hover (always visible) from D-Pad/keyboard focus (`dpadModeActive`-gated). `DpadFocusable`-styled widgets have no equivalent hover signal of their own, so gating `state.focused` this way means they show no visual feedback at all outside D-Pad/TV mode — they rely on their `onSelect` click/tap behavior alone on Desktop and Mobile.
- **Grid column counts are a separate concern from breakpoints.** Card grids (`SearchResultsScreen`, `WatchlistScreen`) size their columns via `SliverGridDelegateWithMaxCrossAxisExtent`, capped at `context.appCardSizes.gridMaxWidth`/`heroGridMaxWidth` (§ 1.3) — the column count falls out of available width ÷ that cap automatically, rather than a manually maintained breakpoint-to-column-count table. New card grids should follow this pattern rather than reintroducing a bespoke breakpoint table.
...
- **Hand-built focusables lack accessibility semantics.** `AnimeCard` and `CalendarCard` are `AnimatedContainer` + `DpadFocusable` compositions rather than real Material controls, and get `dpad`'s full D-pad/keyboard focus behavior (region-escape, position memory) as a result. `EpisodeTile`'s header, `TorrentTile`, and the `watchlist_cards.dart` family (`HeroCard`, `ListCard`, `WatchlistCard`) are visually the same kind of composition, but built on `HoverFocusBuilder` (a plain Flutter `FocusableActionDetector`) instead — functional for basic focus/keyboard interaction, but without `dpad`'s region system (tracked in `ISSUE_BACKLOG.md` #10). Either way, none of these carry an explicit `Semantics(button: true, label: ...)`, so screen readers (TalkBack/VoiceOver) announce nothing meaningful when focus lands on them.
...
- **Border radius outliers to the "12px items / 24px panels" rule in § 1.3:**
  - `BatchEpisodePickerOverlay` and `TorrentSearchModal` (both genuine modals) use 16px, not 24px — this is now one shared literal inside `SelectionModal` rather than two independently hardcoded values, so resolving it is a one-line change whenever this item is formally addressed.
...
---
*Last reviewed against the codebase: 2026-08-11. Added a palette color, a blur/radius/card-size value, or a D-pad pattern? Update this file — see [CLAUDE.md](CLAUDE.md) § 2's Living Documentation Rule.*
