...
> **Covers:** the strict, enforced technical constraints on all Flutter/Dart code in this repo — performance, state management, caching, and code-generation quality standards. **See also:** [CLAUDE.md](CLAUDE.md) for project overview, working norms, and the doc-suite index; [ARCHITECTURE.md](ARCHITECTURE.md) for where code implementing these rules lives; [DESIGN.md](DESIGN.md) for the UI/UX rules these performance constraints support.
...
Before considering any non-trivial change finished, check it against [CLAUDE.md](CLAUDE.md)'s Living Documentation Rule — a change that adds a dependency, a folder, a cache, a native bridge, or a design token isn't done until the matching doc is updated (or flagged) alongside it.

---
*Governed by [CLAUDE.md](CLAUDE.md)'s Living Documentation Rule. Last reviewed against the codebase: 2026-07-28. Changed a performance rule, a caching guideline, or a code-quality directive? Update this file — and check whether [CONTRIBUTING.md](CONTRIBUTING.md)'s PR checklist needs the same update.*
