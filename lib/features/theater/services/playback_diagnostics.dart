...
/// Track C, 3a: read-only diagnostic instrumentation for the mpv
/// permanent-freeze bug reported after ~5-10 minutes of PAUSED playback,
/// where scrubbing afterward fails to resume frame rendering.
///
...
/// ranged GET on resume) can be verified against real device logs rather
/// than asserted from symptoms alone.
///
/// See [AppLogger.logsDirectoryPath] for where these lines end up on
/// disk. If the reconnect tuning alone resolves the freeze, this class
/// can eventually be trimmed to error-path-only logging or removed
/// entirely; if it doesn't, these logs are what a follow-up app-level
/// reconnect watchdog would need to be designed against — specifically,
...
class PlaybackDiagnostics {
  ...
  void _onPlayingChanged(bool playing) {
    if (playing) {
      _pollTimer?.cancel();
      _pollTimer = null;
      // ── Resume: log immediately, then again after a short delay so the
      // pair of lines shows whether position is actually advancing or the
      // freeze reproduced on this specific resume. ──
