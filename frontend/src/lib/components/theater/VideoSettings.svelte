<script lang="ts">
  ...
  // FIX: was untyped `let closeTimeout: number` — use ReturnType for cross-platform safety
...
<div
  class="absolute bottom-16 right-4 z-50 w-64 bg-black/80 border-zinc-800/80 border-2 rounded-lg p-5
         text-white shadow-xl backdrop-blur-sm transition-all duration-300 ease-in-out
         {SettingsOpen
...
>
  {#if SettingsView === "main"}
    ...
    >
      <span class="mr-1"><Captions size={20} /></span>
      <span>Subtitles</span>
      <span class="absolute right-7 text-gray-700 truncate">
        {#if SelectedSub}{SelectedSub?.lang} ({SelectedSub?.title}){:else}None{/if}
      </span>
    </div>
    ...
    >
      <span class="mr-1"><AudioLines size={20} /></span>
      <span>Audio Tracks</span>
      <span class="absolute right-7 text-gray-700">
        {#if SelectedAudio}{SelectedAudio?.lang}{:else}???{/if}
      </span>
    </div>
  {:else if SettingsView === "subs"}
    <button
      class="text-sm mb-2 p-2 hover:cursor-pointer rounded-md hover:bg-white/10 flex row top-0 left-0 text-gray-500"
    ...
    <div class="max-h-48 overflow-y-auto flex flex-col gap-1 custom-scrollbar">
      ...
      {#each animeData?.subtitles ?? [] as subtitle}
        <button
          class="text-sm mb-2 p-2 hover:cursor-pointer rounded-md hover:bg-white/10 w-full text-left
                 {subtitle.selected ? 'text-sky-400 font-bold' : ''}"
          onclick={() =>
            OnTrackChange(
  ...
  {:else if SettingsView === "audio"}
    ...
    >
      {#each animeData?.audio_tracks ?? [] as audio}
        <button
          class="text-sm mb-2 p-2 hover:cursor-pointer rounded-md hover:bg-white/10 w-full text-left
                 {audio.selected ? 'text-sky-400 font-bold' : ''}"
          onclick={() =>
            OnTrackChange(
