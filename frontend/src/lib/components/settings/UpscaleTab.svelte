...
<div class="animate-in fade-in slide-in-from-right-4 duration-300 pb-4">
  ...
  <div class="space-y-6">
    ...
    <div class="space-y-3">
      {#each targetResolutions as resolution}
        ...
        >
          <div class="flex flex-col text-left">
            <span
              class="font-bold {upscalingResolution.id === resolution.id
                ? 'text-primary'
                : 'text-main'}"
            >
              {resolution.label}
      ...
      <button
        class="w-full flex items-center p-4 rounded-xl border-2 transition-all duration-200 {upscalingResolution.id === '4'
          ? 'border-primary bg-primary/10'
          : 'border-border bg-base hover:border-gray-500 text-left'} {upscalingMethod === ''
          ? 'opacity-40 cursor-not-allowed select-none'
          : ''}"
    ...
    >
      <div class="relative flex items-center">
        ...
        <input
          type="number"
          disabled={upscalingResolution.id != "4" || upscalingMethod === ""}
